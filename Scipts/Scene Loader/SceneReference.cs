// Editor Settings
//#define SR_USING_LEVELING
#define SR_USING_FOLDOUT
...
public class SceneReference : ISerializationCallbackReceiver
{
    ...
    {
#if UNITY_EDITOR
        return IsValidSceneAsset ? _sceneAsset.name : "NULL";
#else
...
public class SceneReferencePropertyDrawer : PropertyDrawer
{
    ...
    private static readonly float _paddedLine = _lineHeight + PadSize;
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        EditorGUI.BeginProperty(position, GUIContent.none, property);
        {
            ...
            property.isExpanded = EditorGUI.Foldout(new Rect(position.x, position.y, position.width, _lineHeight),
                property.isExpanded, label);
#else
            // Here we add the foldout using a single line height, the label and change
            // the value of property.isExpanded
            EditorGUI.LabelField(new Rect(position.x, position.y, position.width, _lineHeight), label);
#endif
            ...
            if (property.isExpanded)
            {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    if (buildScene._scene == null) GetScenePathProperty(property).stringValue = string.Empty;
                }

                position.y += _paddedLine;

                if (!buildScene._assetGuid.Empty())
                {
    ...
    public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
    {
        ...
        return _boxPadding.vertical + _lineHeight * lines + PadSize * (lines - 1) + FooterHeight;
    }
    ...
    private void DrawSceneInfoGUI(Rect position, BuildUtils.BuildScene buildScene, int sceneControlID)
    {
        ...
        else if (buildScene._scene.enabled)
        {
            iconContent = EditorGUIUtility.IconContent("d_winbtn_mac_max");
            labelContent.text = "BuildIndex: " + buildScene._buildIndex;
        ...
        using (new EditorGUI.DisabledScope(readOnly))
        {
            ...
            {
                var isEnabled = buildScene._scene.enabled;
    ...
    private static class BuildUtils
    {
        ...
        public static float _minCheckWait = 3;
        ...
        public struct BuildScene
        {
            ...
            public GUID _assetGuid;
            public string _assetPath;
            public EditorBuildSettingsScene _scene;
        }
        ...
        public static bool IsReadOnly()
        {
            ...
            if (!(timeSinceLastCheck > _minCheckWait)) return _cachedReadonlyVal;
        ...
        public static BuildScene GetBuildScene(Object sceneObject)
        {
            ...
            {
                _buildIndex = -1,
                _assetGuid = new GUID(string.Empty)
            };

            if (sceneObject as SceneAsset == null) return entry;

            entry._assetPath = AssetDatabase.GetAssetPath(sceneObject);
            entry._assetGuid = new GUID(AssetDatabase.AssetPathToGUID(entry._assetPath));

            var scenes = EditorBuildSettings.scenes;
            for (var index = 0; index < scenes.Length; ++index)
            {
                if (!entry._assetGuid.Equals(scenes[index].guid)) continue;

                entry._scene = scenes[index];
                entry._buildIndex = index;
                return entry;
            }
        ...
        public static void SetBuildSceneState(BuildScene buildScene, bool enabled)
        {
            ...
            foreach (var curScene in scenesToModify.Where(curScene => curScene.guid.Equals(buildScene._assetGuid)))
            {
        ...
        public static void AddBuildScene(BuildScene buildScene, bool force = false, bool enabled = true)
        {
            if (force == false)
            {
                var selection = EditorUtility.DisplayDialogComplex(
                    "Add Scene To Build",
                    "You are about to add scene at " + buildScene._assetPath + " To the Build Settings.",
            ...
            var newScene = new EditorBuildSettingsScene(buildScene._assetGuid, enabled);
        ...
        public static void RemoveBuildScene(BuildScene buildScene, bool force = false)
        {
            ...
            {
                var tempScenes = EditorBuildSettings.scenes.ToList();
                tempScenes.RemoveAll(scene => scene.guid.Equals(buildScene._assetGuid));
                EditorBuildSettings.scenes = tempScenes.ToArray();
            }
        }
